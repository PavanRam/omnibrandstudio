...
Design (confirmed over prior discussion, not left implicit):
  ...
  - Primary translation: Claude via ``traced_llm_call`` (alias
    "translation-primary", pinned single-provider — never the raw Anthropic
    SDK, per repo invariant #2).
  - Independent reference (for BLEU / BERTScore-or-semantic_similarity):
    ...
    computed once per variant, since it doesn't depend on Claude's output.
  - Back-translation: locale-dedicated Helsinki-NLP model (es/fr/de), or
    mBART reversed for hi; falls back to Claude (traced_llm_call) if the HF
    call fails outright.
  - Embeddings: sentence-transformers/paraphrase-multilingual-mpnet-base-v2
    via HF Inference Providers — used for the semantic/cosine checks AND for
    the (currently dummy) brand-rule retrieval query embedding.
  - Gate: ALL of {BLEU >= 25, BERTScore-F1-or-semantic_similarity >= 0.84,
    ...
    Claude the specific failed checks as correction feedback. Exhausting
...
# violations found" rather than blocking the gate. See conversation notes:
# treat this the same way the brief treated NLLB before the mBART/Helsinki
# pivot — verify with a probe before trusting it in production.
...
async def _semantic_check(candidate: str, reference: str) -> TranslationCheckResult:
    ...
    if _is_per_token(cand_vec) and _is_per_token(ref_vec):
        ...
        # check — same math, different inputs, per the brief's own flag).
...
# ── Brand rules — dummy Qdrant call, kept as a real (not fake) interface so
# ingestion can plug in later with zero structural change. content_generator
# owns real brand-rule application; this degrades quietly to [] until a
# brand_rules collection actually exists. ──────────────────────────────────


async def _get_brand_rules(state: OmniBrandState, locale: str) -> list[str]:
    try:
        client = get_qdrant()
        query_vec = await _embed(f"brand rules for {locale} marketing translation")
        result = await client.query_points(
            collection_name="brand_rules",
            query=_mean_pool(query_vec),
            query_filter={
                "must": [
                    {"key": "brand_id", "match": {"value": state.get("brand_id")}},
                    {"key": "locale", "match": {"value": locale}},
                ]
            },
            limit=5,
        )
        return [p.payload.get("rule", "") for p in result.points if p.payload]
    except Exception as exc:
...
async def _run_translation_gate(
    ...
    # Independent reference — computed once, doesn't depend on Claude's output.
    ...
    for attempt in range(MAX_RETRIES + 1):
        claude_translation, usage = await traced_llm_call(
            model=model,
            messages=messages,
        ...
        back_translation, bt_cost = await _back_translate(claude_translation, locale, state)
        total_cost += bt_cost

        checks = await _run_checks(claude_translation, reference, back_translation, source)
        safety_violations = await _check_content_safety(claude_translation, locale, state)
        ...
        messages = messages + [
            {"role": "assistant", "content": claude_translation},
            {"role": "user", "content": _build_retry_feedback(checks, safety_violations)},
        ]

    variant["translated_content"] = claude_translation
    ...
    if gate_passed:
        variant["final_content"] = claude_translation
