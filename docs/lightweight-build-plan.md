...
**Status: implemented and verified in lite mode.** The review below records the
gaps found in the original plan and the resolutions used during implementation.
Hosted semantic retrieval still requires a valid provider key and explicit
re-ingestion into the new embedding space.
...
`sentence-transformers` locally instead. Making the LiteLLM embedding path
primary, with hash embeddings only as an explicit degraded fallback, removes the
...
| # | Decision | Rationale |
|---|---|---|
| 1 | Drop `sentence-transformers` (and transitively `torch`/`transformers`/`triton`) as a hard dependency; route real 384-dimensional embeddings through a dedicated traced LiteLLM `/embeddings` call, use TF-IDF as the primary reranker, and retain hash vectors only as a logged degraded mode | This is the single largest image-size win; semantic quality requires a working hosted embedding provider |
...
- [x] Remove `presidio-analyzer`, `presidio-anonymizer` from `pyproject.toml`
  and regenerate `uv.lock`
- [x] Remove `sentence-transformers`; add a dedicated traced LiteLLM
  `/embeddings` call with a controlled hash fallback
- [x] Fix the canonical embedding dimension at 384 and isolate the new vectors
  under versioned collection names (`litellm_v1_384`)
- [x] Update `reranker.py` to use `TFIDFReranker` as primary and drop the
  CrossEncoder path
- [x] Add `observability` and `storage` Compose profiles
- [x] Set OTel off for `make up` and on for `make up-full`
- [x] Add `make up-full` and `make smoke-full`
- [x] Update `README.md`, `docs/run-commands.md`, and
  `docs/implementation-deep-dive.md` for lite/full behavior and re-indexing
- [x] Patch `scripts/smoke_test.py` with lite/full modes, persisted campaign
  setup, status polling, matching-DLQ checks, and optional strict publishing
- [x] Add `.dockerignore` so the host `.venv`, caches, local Chroma data, Git
  metadata, frontend, and documentation are not copied into backend images
- [x] Rebuild `api`/`worker`, inspect image size, and pass `make smoke`

Operator follow-up:

- [ ] Configure the provider key used by LiteLLM's `embedding` alias
- [ ] Re-ingest retained RAG content into the `litellm_v1_384` collections
- [ ] Run `make smoke-full` when the optional observability stack is required
- [ ] Run a reviewed CP4 flow with `--require-publishing` when MailHog delivery
  must be proven
...
- `uv lock` resolved 189 packages and removed the torch, transformers, Triton,
  CUDA, spaCy, Presidio, and sentence-transformers dependency trees.
- Focused lightweight/RAG/smoke tests: **15 passed**.
- Docker Compose validation shows exactly six default services: `postgres`,
  `redis`, `litellm`, `mailhog`, `api`, `worker`.
- `.dockerignore` reduced backend build context from **1.41GB to 8.24MB**.
- The rebuilt shared API/worker image is **1,665,695,189 bytes (~1.67GB)**,
  versus the approximately 14GB image observed before this work.
- Running lite containers confirmed `OTEL_ENABLED=0` for both API and worker.
- `make smoke` passed: API health and metrics were available, the persisted
  campaign reached `awaiting_review`, and no matching DLQ entry was present.
- The full backend suite currently reports **231 passed, 9 failed**; the nine
  failures are pre-existing tests in vector-backend selection, campaign/chat
  routes, golden-dataset/knowledge routes, and personalization, outside this
  lightweight-build change.
